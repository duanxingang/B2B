...
class Pay_EweiShopV2Page extends MobileLoginPage 
{
	...
	public function native_url()
    {
        ...
        if (empty($order))
        {
            echo json_encode(['code'=>0,'message'=>'查询不到该订单信息']);
            exit();
        }
        if ($order['status'] == -1)
        {
            echo json_encode(['code'=>0,'message'=>'该订单已关闭']);
            exit();
        }
        else if (1 <= $order['status'])
        {
            echo json_encode(['code'=>0,'message'=>'该订单已付款']);
            exit();
        }
        ...
        load()->model("payment");
        $setting = uni_setting($_W['uniacid'], array('payment'));
//        $sec = pdo_fetch("select * from " . tablename('ewei_shop_payment') . ' where uniacid=:uniacid and id=4 limit 1', array(':uniacid' => $uniacid));
        $sec = pdo_fetch("select * from " . tablename('ewei_shop_payment') . ' where uniacid=:uniacid  limit 1', array(':uniacid' => $uniacid));
        $param_title = $set['shop']['name'] . '订单';
        $params = array();
        $params['tid'] = $order['ordersn']."-3";
        $params['user'] = $openid;
        $params['fee'] = $order['price'];
        $params['title'] = $param_title;
        $order['price'] = floatval($order['price']);
        ...
        if($wechat['result_code'] == 'SUCCESS' && !empty($wechat['code_url'])){
            $data = $wechat['code_url'];
            $level = 'Q';// 纠错级别：L、M、Q、H
            $size = 10;// 点的大小：1到10,用于手机端4就可以了
            include IA_ROOT.'/addons/ewei_shopv2/plugin/pc/core/mobile/order/phpqrcode/phpqrcode.php';
            $QRcode = new QRcode();
            ob_start();
            $QRcode->png($data,false,$level,$size);
            $imageString = base64_encode(ob_get_contents());
            ob_end_clean();
            $path= IA_ROOT.'/addons/ewei_shopv2/plugin/pc/images/';
            $num = rand(10000, 99999);
            $fileName = time().$num. '.png';
            $QRcode->png($data,$path.$fileName,$level,$size);// 生成本地图片
            $wechat['code_url'] = '/addons/ewei_shopv2/plugin/pc/images/'.$fileName;
            //$wechat['code_url'] = urlencode($wechat['code_url']);
            echo json_encode(['code'=>1,'message'=>'二维码生成成功','wechat'=>$wechat]);
        } else{
            echo json_encode(['code'=>0,'message'=>'二维码生成失败']);
        }
    }
    ...
    public function opay()
    {
        ...
        if( empty($order["istrade"]) )
        {
            if( $order["status"] == -1 )
            {
                header("location: " . mobileUrl("pc/order/detail", array( "id" => $order["id"] )));
                exit();
            }
            if( 1 <= $order["status"] )
            {
