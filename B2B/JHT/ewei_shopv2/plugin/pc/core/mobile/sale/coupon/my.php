...
class My_EweiShopV2Page extends PcMobileLoginPage
{
	protected $member;
	...
	public function main()
	{
		...
		foreach($coupons as $val){
			if($val['used'] == 0){
				if($val['timedays'] > 0){
					$coupontime = $val['gettime']+$val['timedays']*3600*24;
					if($coupontime < $time){
                       $past[] = $val;
				    }else{
				    	$use[] = $val;
		...
		$nav_link_list = array( array('link' => mobileUrl('pc'), 'title' => '首页'), array('link' => mobileUrl('pc.member'), 'title' => '我的商城'), array('title' => '我的优惠劵') );
		$ice_menu_array = array( array('menu_key' => 'index', 'menu_name' => '我的优惠劵', 'menu_url' => mobileUrl('pc.sale.coupon.my')) );
		$member = $this->member;
