...
class Index_EweiShopV2Page extends PcMobileLoginPage
{
	public function main() 
	{
		...
		$keys = ((!(empty($_GPC['keywords'])) ? $_GPC['keywords'] : ''));
...
//		$args = array('pagesize' => 12, 'page' => intval($_GPC['page']), 'isnew' => trim($_GPC['isnew']), 'ishot' => trim($_GPC['ishot']), 'isrecommand' => trim($_GPC['isrecommand']), 'isdiscount' => trim($_GPC['isdiscount']), 'istime' => trim($_GPC['istime']), 'issendfree' => trim($_GPC['issendfree']), 'keywords' => trim($_GPC['keywords']), 'cate' => trim($_GPC['cate']), 'order' => trim($_GPC['order']), 'by' => trim($_GPC['by']), 'priceMin' => trim($_GPC['priceMin']), 'priceMax' => trim($_GPC['priceMax']));
		...
		foreach($glist as $k=>$v){
            ...
            if($v['timeend'] > $time && $v['timestart'] > 0){
                $glist[$k]['timestart1'] = date('Y-m-d',$v['timestart']);
                $glist[$k]['timeend1'] = date('Y-m-d',$v['timeend']);
            }
        }
        ...
        foreach($glist as $key=>$g){
	        foreach($menber_goods as $v){
	        	if($g['id'] == $v['goodsid']){
	                $glist[$key]['total1'] = $v['total'];
	            }
	        }
        }
//        dump($glist);die;
	...
	public function get_history() 
	{
		...
		$sql = 'SELECT f.id,f.goodsid,g.title,g.thumb,g.marketprice,g.productprice,f.createtime,g.merchid,g.discounts FROM ' . tablename('ewei_shop_member_history') . ' f ' . ' left join ' . tablename('ewei_shop_goods') . ' g on f.goodsid = g.id ' . ' where 1 ' . $condition . ' ORDER BY f.`id` DESC LIMIT ' . $psize;
		...
		foreach ($list as &$row ) 
		{
			...
			if($member['level'] > 0){
                ...
                $dis = json_decode($row['discounts'],true);
                //会员等级对应的价格
                if(array_key_exists($gl, $dis) and !empty($dis[$gl])){
                    $goods['marketprice'] = $dis[$gl];
                }
            }
	...
	private function _condition($args) 
	{
        ...
        if($user_level > 0){
            ...
            foreach($goods['list'] as $k=>$v){
                ...
                //会员等级对应的价格
                if(array_key_exists($gl, $dis) and !empty($dis[$gl])){
                    $goods['list'][$k]['marketprice'] = $dis[$gl];
                }
                unset($v['discounts']);
            }
        }
	...
	private function getList($args = array()) 
	{
		global $_W;
		$openid = $_W['openid'];
		$activity = ((!(empty($args['activity'])) ? $args['activity'] : 0));
		...
		if (!(empty($activity))){
			$condition = ' and `uniacid` = :uniacid AND `deleted` = 0 AND `activity` = '.$activity;
		}else{
		...
		if (!(empty($keywords))) 
		{
			$condition .= '  AND (status=1 OR status=0) AND (`title` LIKE :keywords OR `keywords` LIKE :keywords)';
		...
		if (!($random)) 
		{
			$sql = 'SELECT id,discounts,title,thumb,marketprice,productprice,minprice,maxprice,isdiscount,isdiscount_time,isdiscount_discounts,sales,salesreal,total,description,bargain,timestart,timeend FROM ' . tablename('ewei_shop_goods') . ' where 1 ' . $condition . ' ORDER BY ' . $order . ' ' . $orderby . ' LIMIT ' . (($page - 1) * $pagesize) . ',' . $pagesize;
			$total = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_goods') . ' where 1 ' . $condition . ' ', $params);
		}
		else 
		{
			$sql = 'SELECT id,discounts,title,thumb,marketprice,productprice,minprice,maxprice,isdiscount,isdiscount_time,isdiscount_discounts,sales,salesreal,total,description,bargain,timestart,timeend FROM ' . tablename('ewei_shop_goods') . ' where 1 ' . $condition . ' ORDER BY rand() LIMIT ' . $pagesize;
			$total = $pagesize;
		}
		$list = pdo_fetchall($sql, $params);
