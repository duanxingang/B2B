...
class Order_EweiShopV2Model 
{
    ...
    /*2019-8-16 非周期付款的未支付订单自动关闭*/
    public function cancel_no_pay()
    {
        global $_W;
        global $_GPC;
        $condition = " uniacid = {$_W['uniacid']} and status=0 and paytype=0 and btype!=2";
        $sql = "select id,createtime from " . tablename('ewei_shop_order') . "  where {$condition}  ORDER BY createtime ASC";
        $list = pdo_fetchall($sql);
        if($list){
            foreach ($list as $k=>$v){
                //超过两天自动关闭
                if((time()-$v['createtime']) > 3600*24*2){
                    pdo_update('ewei_shop_order', ['status'=>-1,'canceltime'=>time()], array('id' => $v['id']));
                }
            }
        }
    }
