...
class Goods_EweiShopV2Model 
{
	...
	public function getList($args = array( )) 
	{
		...
		//活动专场
        if(isset($args['activity']) && $args['activity']>0){
            $condition .= " and activity= ".$args['activity'];
        }
		...
		if( !$random ) 
		{
			$sql = "SELECT id,title,subtitle,thumb,thumb_url,discounts" . $officsql . ",marketprice,productprice,minprice,maxprice,isdiscount,isdiscount_time,isdiscount_discounts,sales,salesreal,total,description,bargain,`type`,ispresell,`virtual`,hasoption,video,bargain,hascommission,nocommission,commission,commission1_rate,commission1_pay,presellprice\r\n            FROM " . tablename("ewei_shop_goods") . " where  " . $condition . " ORDER BY " . $order . " " . $orderby . " LIMIT " . ($page - 1) * $pagesize . "," . $pagesize;
			$total = pdo_fetchcolumn("select count(*) from " . tablename("ewei_shop_goods") . " where  " . $condition . " ", $params);
		}
		else 
		{
			$sql = "SELECT id,title,thumb,thumb_url,discounts" . $officsql . ",marketprice,productprice,minprice,maxprice,isdiscount,isdiscount_time,isdiscount_discounts,sales,salesreal,total,description,bargain,`type`,ispresell,`virtual`,hasoption,bargain,hascommission,nocommission,commission,commission1_rate,commission1_pay,presellprice\r\n            FROM " . tablename("ewei_shop_goods") . " where  " . $condition . " ORDER BY rand() LIMIT " . $pagesize;
			$total = $pagesize;
		}
    ...
    public function changeStatus()
    {
        try{
            global $_W;
            //不是回收站中的商品
            $condition = "`uniacid` = :uniacid AND `deleted` = 0 and merchid=0";
            $params = array( ":uniacid" => $_W["uniacid"] );
            $sql = "SELECT id,marketprice,total,status FROM " . tablename("ewei_shop_goods") . " where  " . $condition;
            $list = pdo_fetchall($sql, $params);
            foreach ($list as $k=>$v){
                if($v['marketprice']>0 && $v['total']>0){
                    //有价格有库存上架
                    $status = 1;
                }else{
                    //否则下架
                ...
                pdo_update("ewei_shop_goods", array( "status" =>$status), array( "id" => $v["id"]));
            }
            return true;
        }catch (Exception $e){
            return true;
    ...
    //erp没传的库存自动改为0
    public function changeZero($inCode)
    {
        try{
            global $_W;
            if(empty($inCode))return true;
            //不是回收站中的商品
            $condition = "`uniacid` = :uniacid AND `deleted` = 0 and merchid=0";
            $params = array( ":uniacid" => $_W["uniacid"] );
            $sql = "SELECT id,total,goodsisn FROM " . tablename("ewei_shop_goods") . " where  " . $condition;
            $list = pdo_fetchall($sql, $params);
            foreach($list as $ks=>$vs){
                if(!in_array($vs['goodsisn'], $inCode)){
                    pdo_update("ewei_shop_goods", array( "total" =>0), array( "id" => $vs["id"]));
                }
            }
    ...
    //商品类别  经营类别
    public function sellCate($param=false){
        $data = [
            '中药材',
            '中药饮片',
            '中成药',
