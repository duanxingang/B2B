        ...
        if (!$packageid) {
            ...
            if (empty($id)) {
                ...
                if(!empty($quickid)){
                    ...
                    /*2019-8-9修改 区分购物车过来的开票和非票 增加（原本没有）搜索条件 and g.isbill={$type} */
                    $sql = 'SELECT c.goodsid,c.total,g.maxbuy,g.type,g.intervalfloor,g.intervalprice,g.issendfree,g.isnodiscount,g.ispresell,g.presellprice as gpprice,o.presellprice,g.preselltimeend,g.presellsendstatrttime,g.presellsendtime,g.presellsendtype'
                        . ',g.weight,o.weight as optionweight,g.title,g.thumb,ifnull(o.marketprice, g.marketprice) as marketprice,o.title as optiontitle,c.optionid,'
                        . ' g.storeids,g.isverify,g.isforceverifystore,g.deduct,g.manydeduct,g.virtual,o.virtual as optionvirtual,discounts,'
                        ...
                        . ' left join ' . tablename('ewei_shop_goods') . ' g on c.goodsid = g.id '
                        . ' left join ' . tablename('ewei_shop_goods_option') . ' o on c.optionid = o.id '
                        . " where c.openid=:openid and c.selected=1 and  c.deleted=0 and c.uniacid=:uniacid and g.isbill={$type}  order by c.id desc";
                    $goods = pdo_fetchall($sql, array(':uniacid' => $uniacid, ':openid' => $openid));
                } elseif (p('exchange')) {
                    $sql = 'SELECT c.goodsid,c.total,g.maxbuy,g.type,g.intervalfloor,g.intervalprice,g.issendfree,g.isnodiscount,g.ispresell,g.presellprice as gpprice,o.presellprice,g.preselltimeend,g.presellsendstatrttime,g.presellsendtime,g.presellsendtype'
                        . ',g.weight,o.weight as optionweight,g.title,g.thumb,ifnull(o.marketprice, g.marketprice) as marketprice,o.title as optiontitle,c.optionid,'
...
//                    自定义表单
            ...
            foreach ($goods as &$g) {
                ...
                if($member['level'] > 0){
                    ...
                    $dis = json_decode($g['discounts'],true);
                    //会员等级对应的价格
                    if(array_key_exists($gl, $dis) and !empty($dis[$gl])) {
                        $g['marketprice'] = $dis[$gl];
                    }
                }
...
EOF;
    ...
    function submit()
    {
        ...
        if($type == 0){
            ...
            if(!empty($data['shop_order_start'])){
                if($_GPC['real_price'] < $data['shop_order_start']){
                    show_json(0,'商城起订金额为'.$data['shop_order_start'].'元');
                }
            }
        }elseif($type == 1){
            ...
            $time .= ' 16:00:00';
            $num = strtotime($time);
            $orders = pdo_fetchall("select id,ordersn,status from " . tablename("ewei_shop_order") . " where uniacid=:uniacid and openid=:openid and status=1 and createtime>:num and paytime>:num", array(":uniacid" => $uniacid, ":openid" => $openid, ":num"=>$num));
            if(empty($orders)){
                $data = m('common')->getSysset('trade');
                if(!empty($data['shop_order_start'])){
                    if($_GPC['real_price'] < $data['shop_order_start']){
        ...
        //是否存在周期付款并超出期限的订单
        $orderl = pdo_fetchall("select id,ordersn,status,paytype,createtime,btype from " . tablename("ewei_shop_order") . " where paytype=0 and btype=2 and uniacid=:uniacid and openid=:openid order by createtime ASC", array(":uniacid" => $uniacid, ":openid" => $openid ));
        foreach($orderl as $kk=>$vv){
            $t = round((time() - $vv['createtime']) / 86400);
//            if($t >= $member['branch_day']){
//                show_json(0,'您有周期付款订单超出期限未付款，请先完成付款！订单号：'.$vv['ordersn']);
//            }
        ...
        foreach($goods as $ka=>$va){
            $newGoods[$ka] = pdo_getcolumn('ewei_shop_goods', array('id' => $va['goodsid']), 'isbill');
        }
        if (empty($newGoods) || !is_array($newGoods)) {
            show_json(0, '未找到任何商品');
        }
        if(in_array(0,$newGoods) && in_array(1,$newGoods)){
            show_json(0, '同一订单里不能包含开票和不开票两种商品哦^_^');
        }
        $ids = implode(',',array_column($goods, 'goodsid'));
...
//                $discountprice=0;
        ...
        /*2019-8-5 修改多商户订单分开问题，如要分开把下方注释内容打开即可*/
        $multiple_order = 0;
...
//            $multiple_order = 1;
//        }
        /*2019-8-5 修改多商户订单分开问题，如要分开把上方注释内容打开即可*/

        //生成订单号
        if ($ismerch > 0) {
            $ordersn = m('common')->createNO('order', 'ordersn', 'ME',1);
        } else {
            $ordersn = m('common')->createNO('order', 'ordersn', 'SH',1);
        }
        ...
        if (!is_null($_SESSION['exchangeprice']) && !empty($_SESSION['exchange']) && p('exchange')){
            ...
            $order['ordersn'] = m('common')->createNO('order', 'ordersn', 'DH',1);
...
//        }
        ...
        if(in_array(0,$newGoods) && !in_array(1,$newGoods)){
            $order['isbill'] = 0;
        }elseif(!in_array(0,$newGoods) && in_array(1,$newGoods)){
            $order['isbill'] = 1;
        }else{
            $order['isbill'] = 0;
        }
...
//自定义表单
        ...
        if ($multiple_order == 0) {
            ...
            foreach ($merch_array as $key => $value) {
                ...
                //生成子订单号
                $order['ordersn'] = m('common')->createNO('order', 'ordersn', $order_head,1);
...
//                pdo_query("INSERT INTO ".tablename('ewei_shop_exchange_record')."(`key`,`uniacid`,`title`,`serial`) VALUES('{$_SESSION['exchange_key']}','{$_W['uniacid']}','{$_SESSION['exchangetitle']}','{$_SESSION['exchangeserial']}')");
        ...
        if ($_GPC['fromcart'] == 1) {
            //删除购物车
            pdo_query('update ' . tablename('ewei_shop_member_cart') . ' set deleted=1 where  openid=:openid and uniacid=:uniacid and selected=1 and goodsid IN ('.$ids.')', array(':uniacid' => $uniacid, ':openid' => $openid));
        }
