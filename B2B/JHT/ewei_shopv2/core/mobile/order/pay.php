...
class Pay_EweiShopV2Page extends MobilePage
{
	...
	public function __construct(){
		...
		if(!empty($_SESSION['wechat_openid'])){   
			$this->openid = $_SESSION['wechat_openid'];
		}else{                                         
			$openider = pdo_fetchcolumn("SELECT openid FROM " . tablename("ewei_shop_order") . " WHERE id = :id AND uniacid = :uniacid LIMIT 1", array( ":id" => $orderid, ":uniacid" => $_W["uniacid"] ));   
			$_SESSION['wechat_openid'] = $openider; 
			$this->openid = $_SESSION['wechat_openid'];      
		}
	}
	...
	public function main() 
	{
		...
		if( !empty($ispeerpay) ) 
		{
			if( pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename("ewei_shop_order_peerpay_payinfo") . " WHERE openid = :openid AND pid = :pid", array( ":openid" => $openid, ":pid" => $ispeerpay["id"] )) ) 
			{
		...
		//对公支付
        $banks = array( "success" => false );
        if( isset($set["pay"]) && $set["pay"]["banks"] == 1 )
        {
            $banks = array( "success" => true, "banks_info" => $sec["banks"] );
        }

        /*2019-8-9修改 区分订单的开票和非票 增加（原本没有）内容  开始 */
        //非票对公支付
        $banks2 = array( "success" => false );
        if( isset($set["pay"]) && $set["pay"]["banks2"] == 1 )
        {
            $banks2 = array( "success" => true, "banks_info2" => $sec["banks2"] );
        }
        /*2019-8-9修改 区分订单的开票和非票 增加（原本没有）内容  结束 */


        //周期付款
        $branch = array( "success" => false );
        if( isset($set["pay"]) && $set["pay"]["branch"] == 1 )
        {
            if(!empty($member['branch_day']) && unserialize($member['branch_day'])['is_branch']==1){
                $branch = array( "success" => true);
            }
        }
		...
		{   
			...
			$params["openid"] = trim($_W['fans']['openid']);
	        ...
	        $h5pay = m('common')->payh5($params, $options, 0);
	        $redirect_url = mobileUrl('order/detail',array('id'=>$orderid)); //支付成功后跳转地址
			$redirect_url = urlencode($_W['siteroot'].'app/'.substr($redirect_url,2));
			...
			if( $type == "wechat" ) 
			{
				...
				if( !is_error($payquery) || !is_error($payquery_jie) || !empty($ispeerpay) ) 
				{
					...
					if( !empty($ispeerpay) ) 
					{
						...
						pdo_update("ewei_shop_order_peerpay_payinfo", $udata, array( "pid" => $ispeerpay["id"], "openid" => $openid ));
					}
					$pay_result = m("order")->payResult($ret);
					@session_start();
					$_SESSION[EWEI_SHOPV2_PREFIX . "_order_pay_complete"] = 1;
	...
	public function success() 
	{
		...
		$openider = $_W['openid'];
		$peerpay = pdo_fetch("SELECT orderid FROM " . tablename("ewei_shop_order_peerpay") . " WHERE uniacid=:uniacid and orderid=:orderid LIMIT 1", array( ":orderid" => $orderid, ":uniacid" => $_W["uniacid"] ));
		if(!empty($peerpay)){
			$this->orderpeerpay($orderid,$openider);
		}
		include($this->template());
	}
    ...
    public function opay()
    {
        ...
        if($type == 'branch') {
            if (isset($set["pay"]) && $set["pay"]["branch"] == 1) {
//                $branch_member_level = explode(',', $sec['branch']['branch_member_level']);
//                if (in_array($member['level'], $branch_member_level)) {
//                    $branch = array("success" => true,"branch_info"=>$member['branch_day']);
//                }
                if(!empty($member['branch_day']) && unserialize($member['branch_day'])['is_branch']==1){
                    $branch = array("success" => true,"branch_info"=>unserialize($member['branch_day'])['days']);
                }
            }
        }
    ...
    public function cpay()
    {
        try{
            ...
            if($type==1 && empty($images)){
                show_json(0,'请上传转账截图哦^_^');
            }
            if( empty($order) )
            {
                show_json(0,'订单不存在');
            ...
            if($order['status'] != 0){
                show_json(0,'订单状态不是未支付状态');
            }
            if($order['btype'] == 1){
                show_json(0,'该订单已经选择使用对公付款');
            }
            if($order['btype'] == 2){
                show_json(0,'该订单已经选择使用周期付款');
            }
            ...
            if(!empty($images)){
                if(empty($order['invoice_img'])){
                    ...
                    $img = unserialize($order['invoice_img']);
                    $img['pay_img'] = ['0'=>$images];
                }
                $data['invoice_img'] = serialize($img);
            }
            ...
            show_json(0,'网络错误，请稍后再试~~');
        }
    }
    //2019.08.20  代付
    public function orderpeerpay($orderid,$openid){
       global $_W;
       global $_GPC;
       $ispeerpay = m("order")->checkpeerpay($orderid);
       $member = m("member")->getMember($openid, true);
       pdo_update("ewei_shop_order_peerpay", array( "peerpay_type" => 1 ), array( "orderid" => $orderid));
       $order = pdo_fetch("SELECT ordersn,wechatpay FROM " . tablename("ewei_shop_order") . " WHERE `uniacid`=:uniacid AND `id`=:id limit 1", array( ":uniacid" => $_W['uniacid'],":id" => $orderid ));
       ...
       $tid = $order["ordersn"] . '-' . $order['wechatpay'];
       	 m("order")->peerStatus(array( "pid" => $ispeerpay['id'], "uid" => $member['id'], "uname" => $member['nickname'], "usay" => "", "price" => $log['fee'], "createtime" => time(), "headimg" => $member['avatar'], "openid" => $member['openid'], "usay" => "代付", "tid" => $tid));
