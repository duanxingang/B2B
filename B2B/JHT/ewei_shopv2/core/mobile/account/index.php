...
class Index_EweiShopV2Page extends Page
{
	...
	public function login()
	{
		...
		if ($_W['ispost']) {
			...
			$cookie = base64_encode(json_encode(['mobile'=>$mobile,'pwd'=>$pwd]));
			isetcookie("uLog", $cookie, time()+3600*24*7);

			//是否注册企业信息
			if(empty($member["company"]) && empty($member["business_card"]) && empty($member["business_img"])){
				$result['message'] = '登录成功';
				$result['is_reg'] = 1;
				$result['app_uid'] = $member["id"];
		...
		$us = [];
		if(!empty($_GPC['uLog'])){
			$us = json_decode(base64_decode($_GPC['uLog']),true);
		}
	...
	protected function rf($type)
	{
		...
		if ($_W['ispost']) {
			...
			$member = pdo_fetch('select id,openid,mobile,pwd,salt,email from ' . tablename('ewei_shop_member') . ' where mobile=:mobile and mobileverify=1 and uniacid=:uniacid limit 1', array(':mobile' => $mobile, ':uniacid' => $_W['uniacid']));
			...
			if (empty($type)) {
				...
				$data = array('uniacid' => $_W['uniacid'],'email' => $email, 'mobile' => $mobile, 'email' => $email, 'nickname' => $nickname, 'openid' => $openid, 'pwd' => md5($pwd . $salt), 'salt' => $salt, 'createtime' => time(), 'mobileverify' => 1, 'comefrom' => 'mobile');
			...
			unset($_SESSION[$key]);
			$result['message'] = empty($type) ? '填写企业信息' : '密码重置成功';
			$result['app_uid'] = $uid;
			show_json(1, $result);
		}
