...
/*
	...
	function payset() {
        ...
        $sec['branch']['branch_member_level'] = explode(',',$sec['branch']['branch_member_level']);
		...
		if ($_W['ispost']) {
            ...
            //对公付款
            $sec['banks']['banks_card'] = trim($_GPC['data']['banks_card']);
            $sec['banks']['banks_name'] = trim($_GPC['data']['banks_name']);
            $sec['banks']['banks_user_name'] = trim($_GPC['data']['banks_user_name']);

            //非票对公付款
            $sec['banks2']['banks_card2'] = trim($_GPC['data']['banks_card2']);
            $sec['banks2']['banks_name2'] = trim($_GPC['data']['banks_name2']);
            $sec['banks2']['banks_user_name2'] = trim($_GPC['data']['banks_user_name2']);

            //周期付款
//            $sec['branch']['branch_member_level'] = implode(",", $_GPC['data']['branch_member_level']);
            ...
            $data['banks'] = intval($inputdata['banks']);
            $data['banks2'] = intval($inputdata['banks2']);
            $data['branch'] = intval($inputdata['branch']);
		...
		//会员等级
        $member_level = pdo_fetchall("SELECT id,levelname FROM ".tablename('ewei_shop_member_level')." WHERE uniacid=:uniacid and enabled = 1 order by id asc",array(':uniacid'=>$_W['uniacid']));
        array_unshift($member_level,['id'=>'0','levelname'=>'普通会员']);
