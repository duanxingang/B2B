...
class Detail_EweiShopV2Page extends WebPage {
    ...
    function main() {
        ...
        if(!empty($item['invoice_img'])){
            $pay_images = unserialize($item['invoice_img'])['pay_img'];
        }
        ...
        $item['btype_time'] = 0;
        if($item['btype'] == 2){
            $ts = round((time() - $item['createtime']) / 86400);
            if(!empty($member['branch_day']) && $ts < unserialize($member['branch_day'])['days']){
                $item['btype_time'] = (unserialize($member['branch_day'])['days'] - $ts);
            }
        }
...
//                $r['optionname'] = $option_title['title'];
            ...
            //是否有退货数
            $refund2 = pdo_fetch("SELECT status as rstatus,goodsid,refundnum FROM " . tablename('ewei_shop_order_refund') ." where status>0 and goodsid={$r['id']} and orderid={$id} and refundnum>0 order by id desc");
            $r['refund2'] = $refund2;
            $total += $r['total'];
            $export_num += $r['export_num'];
