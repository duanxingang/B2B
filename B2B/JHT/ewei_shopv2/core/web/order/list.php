...
//                $condition .= " AND (m.realname=:keyword or m.mobile=:keyword or m.nickname=:keyword or m.company=:keyword or m.contact_name=:keyword or m.contact_phone=:keyword)";
                if(empty($_GPC['isprecise']))
                {
                    $condition .= " AND (locate(:keyword,m.realname)>0 or locate(:keyword,m.mobile)>0 or locate(:keyword,m.nickname)>0 or locate(:keyword,m.company)>0 or locate(:keyword,m.contact_name)>0 or locate(:keyword,m.contact_phone)>0)";
                }else
                {
                    $condition .= " AND (m.realname=:keyword or m.mobile=:keyword or m.nickname=:keyword or m.company=:keyword or m.contact_name=:keyword or m.contact_phone=:keyword)";
                }
...
//                $memberCondition = " AND (nickname LIKE '%".$_GPC['keyword']."%' OR realname LIKE '%".$_GPC['keyword']."%' OR mobile LIKE '%".$_GPC['keyword']."%') ";
        ...
        if (!empty($list)) {
            ...
            foreach ($list as &$value) {
                ...
                //订单商品
                $order_goods = pdo_fetchall('select g.id,g.title,og.title as gtitle,g.thumb,g.invoice,g.goodssn,og.goodssn as option_goodssn, g.productsn,og.productsn as option_productsn, og.total,og.export_num,og.status1,og.erp_trade_no,
                    ...
                    og.diyformfields,op.specs,g.merchid,og.seckill,og.seckill_taskid,og.seckill_roomid,g.ispresell,g.costprice,op.costprice as option_costprice,og.expresssn,og.expresscom,og.express,og.sendtype,g.status as giftstatus,og.single_refundid,og.single_refundstate,og.id as ogid,og.nocommission,og.orderid from ' . tablename('ewei_shop_order_goods') . ' og '
                ...
                foreach ($order_goods as &$og) {
                    ...
                    $og['dan_price'] = number_format(($og['price'] / $og['total']),2);
                ...
                if($value['btype'] == 2){
                    ...
                    $ts = round((time() - $value['createtime']) / 86400);
//                    if($ts < $member['branch_day']){
//                        $value['btype_time'] = ($member['branch_day'] - $ts);
//                    }
                    if(!empty($member['branch_day']) && $ts < unserialize($member['branch_day'])['days']){
                        $value['btype_time'] = (unserialize($member['branch_day'])['days'] - $ts);
                    }
                }
            ...
            $omember = pdo_fetchall('select openid,nickname,id as mid,realname as mrealname,mobile as mmobile,uid,company from '.tablename('ewei_shop_member').' where openid in('.$oopenid.') and uniacid = :uniacid',array(':uniacid'=>$_W['uniacid']),'openid');
            ...
            foreach($list as $lk => $lv){
                $list[$lk]['company'] = $omember[$lv['openid']]['company'];
        ...
        foreach($list as $key=>$v){
            foreach($v['goods'] as $rg){
                if($v['id'] == $rg['orderid']){
                    $list[$key]['export_nums'] += $rg['export_num'];
                    $list[$key]['totals'] += $rg['total'];
                }
            }
            if($v['status'] == "已完成" || $v['status'] == "待收货"){
                 $list[$key]['statuss'] = 2;
            }
        }
        ...
        if ($_GPC['export'] == 1) {
            ...
            foreach ($list as &$row) {
                ...
                $row['refundes'] = '';
                if($row['refundid']>0){
                    if($row['ispartrefund']==0){
                        $row['refundes'] = '部分退款';
                        foreach($row['goods'] as $ks=>$vs){
                            $row['goods'][$ks]['refundnum'] = pdo_getcolumn('ewei_shop_order_refund', array('goodsid' => $vs['id'],'orderid'=>$row['id'],'uniacid'=>$_W['uniacid']), 'refundnum');
                            IF($row['goods'][$ks]['refundnum'] == FALSE){
                                $row['goods'][$ks]['refundnum'] = '';
                            }
                        }
                    }elseif($row['ispartrefund']==1){
                        $row['refundes'] = '整单退款';
                    }
                }
            }
...
//            ));
        ...
        if ($condition != ' o.uniacid = :uniacid and o.ismr=0 and o.deleted=0 and o.isparent=0' || !empty($sqlcondition)){
            if ($searchfield == 'member') {
                ...
                $priceCondition .= " AND (nickname LIKE '%".$_GPC['keyword']."%' OR realname LIKE '%".$_GPC['keyword']."%' OR mobile LIKE '%".$_GPC['keyword']."%' OR company LIKE '%".$_GPC['keyword']."%' OR contact_name LIKE '%".$_GPC['keyword']."%' OR contact_phone LIKE '%".$_GPC['keyword']."%') ";
                $priceCondition1= " AND (nickname LIKE '%".$_GPC['keyword']."%' OR realname LIKE '%".$_GPC['keyword']."%' OR mobile LIKE '%".$_GPC['keyword']."%' OR company LIKE '%".$_GPC['keyword']."%' OR contact_name LIKE '%".$_GPC['keyword']."%' OR contact_phone LIKE '%".$_GPC['keyword']."%') ";
                ...
                if (!empty($openidArr)) {
                    ...
                    $orderPrice = pdo_fetch('SELECT count(1) as count,sum(price) as sumprice FROM '.tablename('ewei_shop_order').' o WHERE o.uniacid = '.$_W['uniacid'].' AND o.deleted=0 AND o.paytype>0 AND o.isparent=0 AND o.openid IN ('.$inOpenid.')'.$timeCondition . $statuscondition) ;

                    //2019-7-30 订单付款金额
                    $orderPayPrice = pdo_fetch('SELECT sum(price) as sumprice2 FROM '.tablename('ewei_shop_order').' o WHERE o.uniacid = '.$_W['uniacid'].' AND o.deleted=0 AND o.paytype>0 AND o.isparent=0 AND o.openid IN ('.$inOpenid.')') ;
                    //2019-7-30 订单退款金额
                    $orderidArr = pdo_fetchall('SELECT id FROM '.tablename('ewei_shop_order').' WHERE uniacid = '.$_W['uniacid'].' AND openid IN ('.$inOpenid.')') ;
                    if(!empty($orderidArr)){
                        foreach ($orderidArr as $oid) {
                            $oids[] = $oid['id'];
                        }
                        $inOid = "'".implode('\',\'',$oids)."'";
                        $rePrice = pdo_fetch('SELECT sum(price) as rprice FROM '.tablename('ewei_shop_order_refund').' WHERE uniacid = '.$_W['uniacid'].' AND status=1  AND orderid IN ('.$inOid.')') ;
                    }else{
                        $rePrice['rprice'] = 0;
                    }

                }else{
                    $orderPayPrice['sumprice2'] = 0;
                    $rePrice['rprice'] = 0;
                    $orderPrice['sumprice'] = 0;
                }

                $payMoney = number_format($orderPayPrice['sumprice2'],2);
                $reMoney = number_format($rePrice['rprice'],2);
                ...
                $orderPrice = pdo_fetch('SELECT count(1) as count,sum(o.price) as sumprice FROM '.tablename('ewei_shop_order').
                    ' o left join '.tablename('ewei_shop_member_address').' a on o.addressid = a.id WHERE o.uniacid = '.$_W['uniacid'].'  AND o.deleted=0 AND o.paytype>0 AND o.isparent=0 '.$priceCondition.$statuscondition);
                $totalmoney = $orderPrice['sumprice'];
                $total = $orderPrice['count'];
                ...
                //2019-7-30 订单付款金额
                $orderPayPrice = pdo_fetch('SELECT sum(o.price) as sumprice2 FROM '.tablename('ewei_shop_order').
                    ' o left join '.tablename('ewei_shop_member_address').' a on o.addressid = a.id WHERE o.uniacid = '.$_W['uniacid'].'  AND o.deleted=0 AND o.isparent=0 AND o.paytype>0');
                //2019-7-30 订单退款金额
                $orderidArr = pdo_fetchall('SELECT id FROM '.tablename('ewei_shop_order').
                    ' o left join '.tablename('ewei_shop_member_address').' a on o.addressid = a.id WHERE o.uniacid = '.$_W['uniacid'].'  AND o.deleted=0 AND o.isparent=0 AND o.paytype>0');
                if(!empty($orderidArr)){
                    foreach ($orderidArr as $oid) {
                        $oids[] = $oid['id'];
                ...
                //2019-7-30 订单付款金额
                $orderPayPrice = pdo_fetch('SELECT sum(price) as sumprice2 FROM '.tablename('ewei_shop_order').' WHERE uniacid = '.$_W['uniacid'].' AND deleted=0 AND paytype>0 AND isparent=0 and ismr=0') ;
                //2019-7-30 订单退款金额
                $orderidArr = pdo_fetchall('SELECT id FROM '.tablename('ewei_shop_order').' WHERE uniacid = '.$_W['uniacid'].' AND deleted=0 AND paytype>0 AND isparent=0 and ismr=0') ;
                if(!empty($orderidArr)){
                    foreach ($orderidArr as $oid) {
                        $oids[] = $oid['id'];
                    }
            ...
            $t = pdo_fetch(
                'SELECT COUNT(*) as count, ifnull(sum(price),0) as sumprice   FROM ' . tablename('ewei_shop_order') . " WHERE uniacid = :uniacid and ismr=0 and deleted=0 AND paytype>0 and isparent=0 {$status_condition}", $paras);
            $total = $t['count'];
