    ...
    function main($goodsfrom='sale') {
        ...
        if(empty($_W['shopversion'])){
            ...
            if(!empty($_GPC['goodsfrom'])){
                header('location: ' . webUrl('goods/'. $_GPC['goodsfrom'],array('page'=>$_GPC['page'])));
            }
        }
        ...
        if (!empty($_GPC['keyword'])) {
            ...
            $condition .= ' AND (g.`id` = :id or g.`title` LIKE :keyword or g.`keywords` LIKE :keyword or g.`goodsisn` LIKE :keyword or g.`goodssn` LIKE :keyword or g.`productsn` LIKE :keyword or op.`title` LIKE :keyword or op.`goodssn` LIKE :keyword or op.`productsn` LIKE :keyword';
            if ($merch_plugin) {
