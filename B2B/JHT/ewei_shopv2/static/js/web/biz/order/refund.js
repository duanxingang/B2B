    ...
    modal.init = function(params) {
        ...
        $('.btn-submit').click(function() {
            ...
            if ($('#rtype').val() != 2) {
                ...
                if(goodsid > 0){
                    if (!$('#number').isNumber()) {
                    FoxUI.toast.show('请输入数字数量!');
                    return
                    }
                    if ($('#number').val() < 0) {
                    FoxUI.toast.show('请输入正确的退货数量!');
                    return
                    }
                }else{
                    if (!$('#price').isNumber()) {
            ...
            core.json('order/op/busrefund/submit', {
                ...
                if (ret.status == 1) {
                    location.href = core.getUrl('order/op/busrefund/detail', {
                        id: modal.params.orderid
                    });
                    return
                }
        ...
        $('.btn-cancel').click(function() {
            ...
            FoxUI.confirm('确定您要取消申请?', '', function() {
                ...
                core.json('order/op/busrefund/cancel', {
                    'id': modal.params.orderid,
                    'goodsid':$('#goodsid').val(),
                }, function(postjson) {
        ...
        $('#express_submit').click(function() {
            ...
            core.json('order/op/busrefund/express', {
                id: modal.params.orderid,
        ...
        $('.btn-receive').click(function() {
            ...
            FoxUI.confirm('确认您已经收到换货物品?', '', function() {
                $(this).attr('stop', 1).html('正在处理...');
                core.json('order/op/busrefund/receive', {
                    refundid: modal.params.refundid,
                    id: modal.params.orderid
                }, function(postjson) {
                    if (postjson.status == 1) {
