...
class Coupon_EweiShopV2ComModel extends ComModel 
{
	...
	public function sendMessage($coupon, $send_total, $member, $account = NULL) 
	{
		...
		if( !empty($articles) ) 
		{
			...
			if( is_error($resp) ) 
			{
				...
				$msg = array( "first" => array( "value" => "亲爱的" . $member["nickname"] . "恭喜您获得优惠券", "color" => "#ff0000" ), "keyword1" => array( "title" => "业务类型", "value" => "优惠券通知", "color" => "#000000" ), "keyword2" => array( "title" => "处理进度", "value" => "获得" . $coupon["couponname"], "color" => "#000000" ), "keyword3" => array( "title" => "处理内容", "value" => $desc, "color" => "#4b9528" ), "remark" => array( "value" => "点击查看详情", "color" => "#000000" ) );
				if( !empty($templateid) ) 
				{
	...
	public function setCoupon($row, $time, $withOpenid = true) 
	{
		...
		else 
		{
			if( $time <= $row["timestart"] ) 
			{
				$row["timestr"] = date("Y-m-d", $row["timestart"]) . " 至 " . date("Y-m-d", $row["timeend"]);
			}
			else 
			{
