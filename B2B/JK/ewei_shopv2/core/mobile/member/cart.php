...
class Cart_EweiShopV2Page extends MobileLoginPage 
{
	...
	public function get_list() 
	{
        ...
        if($type == 0){
            $condition .= ' and g.isbill=0';
        }else{
            $condition .= ' and g.isbill=1';
        }
		...
		show_json(1, array( "ischeckall" => $ischeckall, "list" => $list, "total" => $total, "totalprice" => round($totalprice, 2), "merch_user" => $merch_user, "merch" => $merch ,"ts"=>$type));
	}
	...
	public function add() 
	{
		...
		($total <= 0) && ($total = 1);
		$optionid = intval($_GPC['optionid']);
		$goods = pdo_fetch("select id,marketprice,diyformid,diyformtype,diyfields, isverify, `type`,merchid, cannotrefund,hasoption,discounts from " . tablename("ewei_shop_goods") . " where id=:id and uniacid=:uniacid limit 1", array( ":id" => $id, ":uniacid" => $_W["uniacid"] ));
		if( empty($goods) ) 
		{
			show_json(0, "商品未找到");
		}
	...
	private function _validateCartOverLimit() 
	{
		...
		if( 4000 < $listCount + $simpleGoodsCount + $wholesaleCount ) 
		{
			show_json(0, "您的购物车宝贝超过4000个了，建议您先去结算或清理");
		}
	}
