...
class Refund_EweiShopV2Page extends MobileLoginPage {
    ...
    function main() {//2019.5.20
        global $_W, $_GPC;
        $ly = isset($_GPC['ly']) ? $_GPC['ly'] : 'mobile';
    ...
    function submit() {
        ...
        if ($order['refundstate'] == 0 || empty($refunOneGet)) {
            ...
            if($goodsid){
               pdo_update('ewei_shop_order', array('refundid' => $refundid, 'refundstate' => $refundstate, 'ispartrefund' => 0, 'refundtime' => 0), array('id' => $orderid, 'uniacid' => $uniacid)); 
            }else{//如果商品id不存在则为全额退款 0为部分退款   1为全额退款
                pdo_update('ewei_shop_order', array('refundid' => $refundid, 'refundstate' => $refundstate, 'ispartrefund' => 1), array('id' => $orderid, 'uniacid' => $uniacid));  
            }
    ...
    function cancel() {
        ...
        if($goodsid){
             ...
             pdo_update('ewei_shop_order_goods', $change_goods, array('uniacid' => $uniacid, 'goodsid' => $goodsid));
        }else{
