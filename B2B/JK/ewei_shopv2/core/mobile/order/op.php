...
class Op_EweiShopV2Page extends MobileLoginPage {
    ...
    function finish() {
        ...
        $order = pdo_fetch("select id,status,openid,couponid,price,refundstate,refundid,ispartrefund,ordersn,price from " . tablename('ewei_shop_order') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1'
            , array(':id' => $orderid, ':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']));
        if (empty($order)) {
            show_json(0, '订单未找到');
        }
