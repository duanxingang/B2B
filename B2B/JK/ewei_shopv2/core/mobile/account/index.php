	...
	protected function rf($type)
	{
		...
		if ($_W['ispost']) {
			...
			if (empty($type)) {
				...
				$data = array('uniacid' => $_W['uniacid'],'email' => $email, 'mobile' => $mobile, 'nickname' => $nickname, 'openid' => $openid, 'pwd' => md5($pwd . $salt), 'salt' => $salt, 'createtime' => time(), 'mobileverify' => 1, 'comefrom' => 'mobile');
