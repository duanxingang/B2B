...
//经营类别
$sell_cates = m('goods')->sellCate(true);

$price = explode('--',$item['merchantprice']);
$item['makelowprice'] = $price[0];
$item['makehighprice'] = $price[1];

$merch = pdo_fetchall("select id,merchname from ims_ewei_shop_merch_user where uniacid=:uniacid",array(":uniacid"=> $_W['uniacid']));
$merchid = array('id'=>0,'merchname'=>'自营');
...
if( $_W["ispost"] )
{
	$makelowprice = trim($_GPC["makelowprice"]);
    $makehighprice = trim($_GPC["makehighprice"]);
    $merchantprice = $makelowprice.'--'.$makehighprice;
	if( empty($id) ) 
	{
	...
	show_json(1, array( "url" => webUrl("goods/edit", array( "id" => $id,"goodsfrom"=>$goodsfrom,"page"=>$page, "tab" => str_replace("#tab_", "", $_GPC["tab"]) )) ));
}
if( !empty($id) ) 
{
	if( empty($item) ) 
