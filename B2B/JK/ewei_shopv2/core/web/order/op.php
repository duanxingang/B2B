...
class Op_EweiShopV2Page extends WebPage {
        ...
        if (!empty($item['virtual']) && com('virtual')) {
            ...
            if(!empty($images)){
                if(empty($item['invoice_img'])){
                    ...
                    $img = unserialize($item['invoice_img']);
                    $img['pay_img'] = ['0'=>$images];
                }
                $updateorder['invoice_img'] = serialize($img);
            }
    ...
    public function export()
    {
        global $_W,$_GPC;
        $id = $_GPC['id'];
        $order_goods = pdo_fetch('select id,orderid,goodsid,total,export_num,erp_trade_no from '.tablename('ewei_shop_order_goods').' where id = :id limit 1',array(':id'=>$id));
        ...
        if ($_W['ispost']) {
            ...
            $id = $_GPC['id'];
            $export_num = intval($_GPC['export_num']);
            $erp_trade_no = trim($_GPC['erp_trade_no']);

            if (empty($id)) {
                show_json(0, "参数错误！");
            }

            $goods = pdo_fetch('select id,orderid,goodsid from '.tablename('ewei_shop_order_goods').' where id = :id limit 1',array(':id'=>$id));
            $change_data['export_num'] = $export_num;
            $change_data['erp_trade_no'] = $erp_trade_no;
            pdo_update('ewei_shop_order_goods', $change_data, array('id' => $id));
            plog('order.op.export', "修改订单商品发货信息 订单ID: {$goods['orderid']} 商品ID: {$goods['goodsid']}");
            show_json(1);
