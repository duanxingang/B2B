...
class MobilePage extends Page 
{
	...
	public function followBar($diypage = false, $merch = false) 
	{
		...
		{
			$set = $_W["shopset"];
			$followbar = array( "followurl" => $set["share"]["followurl"], "shoplogo" => tomedia($set["shop"]["logo"])?tomedia($set["shop"]["logo"]):tomedia($set["shop"]["pc_logo"]), "shopname" => $set["shop"]["name"], "qrcode" => tomedia($set["share"]["followqrcode"]), "share_member" => false );
			$friend = false;
    ...
    //判断b2b商城用户是否注册，没有就跳转注册
	public function is_company_reg()
    {
        try {
            global $_W;
            $_W['mopenid'] = $_SESSION['wechat_openid'];
            $members = pdo_fetch('select company,business_card,business_img from ' . tablename('ewei_shop_member') . ' where openid=:openid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $_W["mopenid"]));
            if(empty($members["company"]) && empty($members["business_card"]) && empty($members["business_img"])){
                header('location: ' . mobileUrl('member/company/register'));
            }
        } catch (Exception $e) {
    ...
    //判断是否账号密码登陆  页面跳转
	public function is_account_land()
    { 
        global $_W;
        $key = "f40e___ewei_shopv2_member_session_".$_W['uniacid'];
        ...
        $_W['mopenid'] = $_SESSION['wechat_openid'];
        $members = pdo_fetch('select * from ' . tablename('ewei_shop_member') . ' where openid=:openid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $_W["mopenid"]));
        if(empty($members)){
        	header('location: ' . mobileUrl('account/login'));
        }
