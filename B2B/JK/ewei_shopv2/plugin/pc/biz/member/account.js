define(['core'], function (core, tpl, picker) {
        ...
        $('#btnSubmit').click(function () {
            ...
            core.json('pc.account.login', {mobile: $('#mobile').val(), pwd: $('#pwd').val()}, function (ret) {
                ...
                if (ret.status != 1) {
                    ...
                    if(ret.result.is_reg == 1){
                        location.href = core.getUrl('pc.account.reg',{
                            app_uid: ret.result.app_uid,
                        })
                    }else{
                        $('#btnSubmit').html('正在跳转...');
                        setTimeout(function() {
    ...
    modal.initRf = function (params) {
        ...
        $('#btnSubmit').click(function () {
            ...
            var isnum
            if ($('#email').val().length < 7) { //2019.08.13
                FoxUI.toast.show('请输入正确的邮箱地址');
                return
            }
            ...
            if(!modal.type){
                ...
                if(!$("input[type='checkbox']").is(':checked')){
                    FoxUI.toast.show('请认真阅读用户许可协议并同意勾选!');
                    return
                }
            }
        ...
        $('#btnSubmit').click(function () {
            ...
            core.json('pc.member.changepwd', {
                ...
                FoxUI.alert('修改成功!', '', function () {
                    location.href = core.getUrl('pc.member.changepwd');
                })
            }, false, true)
        })
    };
