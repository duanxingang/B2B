...
class Detail_EweiShopV2Page extends PcMobileLoginPage
{
	...
	/*	else 
		...
		if (empty($this->merch_user)) 
		{
			...
			{
				$shopdetail = array(
				    'logo' => !empty($goods['detail_logo']) ? tomedia($goods['detail_logo']) : (!empty($shop['logo'])?tomedia($shop['logo']):tomedia($shop['pc_logo'])),
                    ...
                    'btntext1' => trim($goods['detail_btntext1']),
                    'btnurl1' => (!(empty($goods['detail_btnurl1'])) ? $goods['detail_btnurl1'] : mobileUrl('goods')), 'btntext2' => trim($goods['detail_btntext2']),
		...
		//为富文本内容图片添加上域名路径
        $home = $_W['siteroot'].'/attachment/';
        $goods['content'] = replacePicUrl($goods['content'],$home);
		...
		$hotList = m('goods')->getList(array('page' => 1, 'pagesize' => 10, 'order' => 'salesreal', 'by' => 'DESC'));
        if($member['level'] > 0){
            $gl = 'level'.$member['level'].'_pay';
            foreach($hotList['list'] as $kk=>$vv){
                $dis = json_decode($vv['discounts'],true);
                //会员等级对应的价格
                if(array_key_exists($gl, $dis) and !empty($dis[$gl])){
                    $hotList['list'][$kk]['marketprice'] = $dis[$gl];
                }
            }
        }
        $time = time();
        if($goods['timeend'] > $time && $goods['timestart'] > 0){
            $goods['timestart1'] = date('Y-m-d',$goods['timestart']);
            $goods['timeend1'] = date('Y-m-d',$goods['timeend']);
        }
