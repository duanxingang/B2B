...
class Index_EweiShopV2Page extends PcMobileLoginPage
{
	...
	public function getlist()
	{
		...
		$nav_link_list = array( array('link' => mobileUrl('pc'), 'title' => '首页'), array('link' => mobileUrl('pc.member'), 'title' => '我的商城'), array('title' => '优惠劵') );
		$ice_menu_array = array( array('menu_key' => 'index', 'menu_name' => '优惠劵', 'menu_url' => mobileUrl('pc.sale.coupon')) );
		$member = $this->member;
	...
	public function getwxcard()
	{
		...
		foreach ($wxcard as $i => &$row) {
			...
			if ($row['datetype'] == 'DATE_TYPE_FIX_TIME_RANGE') {
				$title4 = date('Y.m.d', $row['begin_timestamp']) . '至' . date('Y.m.d', $row['end_timestamp']);
			}
			else {
