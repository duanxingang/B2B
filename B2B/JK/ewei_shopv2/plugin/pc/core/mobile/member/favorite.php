...
class Favorite_EweiShopV2Page extends PcMobileLoginPage 
{
	...
	public function get_list() 
	{
		...
		if (!(empty($total))) 
		{
			$sql = 'SELECT f.id,f.goodsid,g.title,g.thumb,g.marketprice,g.productprice,g.merchid,g.discounts FROM ' . tablename('ewei_shop_member_favorite') . ' f ' . ' right join ' . tablename('ewei_shop_goods') . ' g on f.goodsid = g.id ' . ' where 1 ' . $condition . ' ORDER BY `id` DESC LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize;
