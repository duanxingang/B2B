...
class EweiShopWechatPay 
{
	...
	public function order() 
	{
        ...
        $msg = 'date:' . date('Y-m-d H:i:s') . '  ' . $ordersn;
	    $dir = __DIR__ . '/logs';
	    if (! is_dir($dir)) {
	        mkdir($dir, 0777, true);
	    }
		...
		$ordersn = $tid = explode('-',$tid);
        $num = count($tid);
        if($num > 1){
        	$wechatpay = $tid[1];
        	$ordersn = $tid[0];
        	if($wechatpay > 5 ){
        		pdo_update("ewei_shop_order", array( "wechatpay" => $wechatpay , "status" => 1 , "paytime" => time() ), array( "ordersn" => $ordersn));
        	}else{
        		pdo_update("ewei_shop_order", array( "wechatpay" => $wechatpay ), array( "ordersn" => $ordersn));
        	}
        }
		$ordersn = $tid = $tid[0];
		...
		if( !empty($log) && ($log["status"] == "0" || $ispeerpay) && ($log["fee"] == $this->total_fee || $ispeerpay) ) 
		{
			...
			{
				pdo_update("ewei_shop_order", array( "paytype" => 21, "isborrow" => $isborrow, "borrowopenid" => $borrowopenid,"wechatpay"=>$wechatpay, "apppay" => ($this->isapp ? 1 : 0), "transid" => $transaction_id ), array( "ordersn" => $log["tid"], "uniacid" => $log["uniacid"] ));
			}
